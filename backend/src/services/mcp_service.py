...
from backend.src.custom_logger import logger
...
def suggest_onsen_destination() -> str:
    ...
    logger.info(
        f"Tool 'suggest_onsen_destination' called, selected destination: {choice}"
    )
    return choice
...
def estimate_travel_budget(
    ...
    per_day_per_person: int = Field(
        15000, description="1人1日あたりの予算（円、デフォルトは15,000円）"
    ),
) -> int:
    """人数と日数から旅行の予算を見積もります。"""
    budget = days * people * per_day_per_person
    print(
        f"Estimating budget: {days}日 x {people}人 x {per_day_per_person}円 = {budget}円"
    )
    return budget
...
def check_best_season(
    destination: str = Field(
        ..., description="観光地の名前（例：北海道、京都、沖縄など）"
    ),
) -> str:
